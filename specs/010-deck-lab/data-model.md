...
- **Rating:** Heuristic 100-point score, letter, five Axes de deck, strengths, weaknesses, examples, formula version, score meaning and the underlying weighted contribution/factor audit.
- **Build proposal:** Separate kept-from-main, added, removed and unused card counts, all including relevant basic lands. Unused counts include submitted basic lands left outside the proposal. `final` is the complete proposed nonbasic list used for export. The result also includes basic land allocation, previous rating when available, proposed rating and improvement indicator. Every nonbasic card in the proposal comes from the resolved pool.
