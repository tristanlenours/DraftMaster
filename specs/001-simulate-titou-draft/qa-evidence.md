...
| FR-011 — déterminisme                                                                                 | Vecteurs RNG, doubles simulations, projection fonctionnelle, fixture de référence | T014–T015, T036, T038, T041–T043      | Partiellement couvert en US1 (RNG, projection) ; tests stricts US2 prévus |
| FR-012, FR-013 — journal autonome, ordonné et détaillé                                                | Contrats moteur/rapport/relecture et audit indépendant                            | T018–T023, T028–T029, T037–T040, T044 | T018–T023, T028–T029 réussis (journal complet autonome et ordonné, snapshot embarqué) ; replay prévu en US2 |
...
| FR-019 — versions moteur/politiques immuables                                                         | Refus des registres ou décisions incohérents, rapport et replay                   | T018, T024–T029, T037, T041, T044     | T018, T024–T029 réussis (versions fixées à la création, refus de POLICY_MISMATCH, rapport conforme) |
| SC-001, SC-002, SC-003, SC-004, SC-005 — résultat, conservation, atomicité et audit                   | Suites US1/US2/US3, tailles 545/540/360, rapport altéré                           | T019–T046, T050–T051                  | SC-001, SC-003, SC-004 validés pour US1 ; SC-002 et SC-005 en cours (US2/US3) |
...
| Passages gauche/droite/gauche et séquences d'événements continues            | tests rotation, replay et audit                                              | événements attendus/observés              | Validé (rotation.test.ts, simulate-draft.test.ts) |
| Même entrée fonctionnelle, même résultat ; flux RNG indépendants             | vecteurs dorés, déterminisme et référence seed 42                            | versions, seed et digest fonctionnel      | Validé sur US1 (seed 42 reproductible) ; tests formels en US2 |
| Snapshot embarqué et replay autonome sans réseau, RNG ni politique           | contrat et équivalence de replay                                             | journal sérialisé et résultat reconstruit | Prévu en US2                                     |
| Versions moteur/politiques fixées et cohérentes                              | contrats de session, choix, replay et rapport                                | descripteurs des huit sièges              | Validé (contrats de session, simulate-draft, draft-report) |
| Empreinte SHA-256 recalculable et corruption détectée                        | projection, intégrité et mutations ciblées                                   | digest attendu et recalculé               | Validé pour US1 (RFC 8785 canonique sur projection) ; corruption testée en US3 |
...
| `npm run check`                                                             | Format, lint, types, tests fonctionnels disponibles et couverture | Réussi : 15 suites, 109 tests passés, 0 erreur lint/typecheck, couverture V8 lignes 85,95 % |
...
| `npm run test:reference` / `npm run test:replay`                            | Déterminisme et relecture                                         | À raccorder (US2)                                 |
...
- Le MVP US1 est désormais complet, validé et démontrable en CLI (`npm --silent run simulate --`) et via l'interface de domaine TypeScript (`simulateDraft`, `startDraft`, `submitPickRound`).
- La couverture V8 actuelle est diagnostique : 86,42 % des instructions, 77,51 % des branches, 94,20 % des fonctions et 85,95 % des lignes sur l'ensemble du projet. Le moteur de domaine atteint 95,72 % des lignes sur `src/draft/internal/` et 100 % sur `src/draft/index.ts`.
