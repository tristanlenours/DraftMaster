...
if (
  ...
  throw new Error("Usage: node scripts/research/untapped-pair-affinity-benchmark.mjs [--output <path>]");
}
...
const validation = lambdas.map((lambda) => {
  ...
  for (const [validationId, validationBaseSamples] of trainingBaseDrafts) {
    const trainBaseDrafts = trainingBaseDrafts
      .filter(([draftId]) => draftId !== validationId)
    const train = enrichTrainingDrafts(trainBaseDrafts).flatMap(
      ([, samples]) => samples,
    );
...
const report = {
  schemaVersion: "untapped-pair-affinity-benchmark/v1",
  purpose:
    "Exploratory held-out calibration benchmark with regularized directional card-pair priors. This does not reproduce Untapped's proprietary algorithm.",
  split: {
    heldOutDraftId,
