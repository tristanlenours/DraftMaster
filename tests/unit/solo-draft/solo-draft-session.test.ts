...
describe("SoloDraftSession", () => {
  ...
  it("prioritizes on-color lands in AI deck recommendation and maintains a legal 40-card deck", async () => {
    const session = await SoloDraftSession.create({
    ...
    expect(reco.maindeckCardInstanceIds.length + totalBasics).toBe(40);
    expect(totalBasics).toBeGreaterThanOrEqual(10);
    expect(reco.maindeckCardInstanceIds.length).toBeGreaterThanOrEqual(20);
